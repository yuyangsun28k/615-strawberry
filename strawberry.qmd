...
First, we splited the 'Data Item' column into four parts: Fruit, temp1(type of fruit), temp2(market type), temp3(value in currency). Then we divided temp1 into two parts:crop_type and prop_acct.
...
What are the differences process market and fresh market?
Which state yields the most volume of strawberries?
...
# Fresh Market
unique_fresh <- unique(strwb_census$`Fresh Market`)
print(unique_fresh)

plot1_data <- strwb_census %>%
  select(Year, State, `Fresh Market`, Value) %>%
  filter(Year == 2021 & `Fresh Market` == "OPERATIONS WITH SALES")
...
ggplot(plot1_data, aes(x=reorder(State, -Value), y=Value)) + 
  ...
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  labs(x = "States", y = "Count",
title ="Number of Organic Strawberry operations with Sales on Fresh Market in 2021")

# Process Market
unique_process <- unique(strwb_census$`Process Market`)
print(unique_process)

plot11_data <- strwb_census %>%
  select(Year, State, `Process Market`, Value) %>%
  filter(Year == 2021 & `Process Market` == "OPERATIONS WITH SALES")

plot11_data$Value <- as.numeric(plot11_data$Value)

plot11_data <- plot11_data |> arrange(desc(Value))

ggplot(plot11_data, aes(x=reorder(State, -Value), y=Value)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  labs(x = "States", y = "Count",
title ="Number of Organic Strawberry operations with Sales on Process Market in 2021")
print(plot11_data)
```
...
# Process Market
plot22_data <- strwb_census |> 
  select(c(Year, State, `Process Market`, Metric, Value)) |> 
  filter((Year == 2021) & 
...
plot22_data$Value <- as.numeric(gsub(",", "", plot22_data$Value))

plot22_data <- plot22_data |> arrange(desc(Value))

ggplot(plot22_data, aes(x=reorder(State, -Value), y=Value)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  labs(x = "States", y = "Sales",
title ="Organic Strawberry Sales ($) on process market in 2021")
print(plot22_data)
```
...
Pests or Diseases: An outbreak of pests or diseases can drastically reduce yields.
Economic or Social Factors: Changes in demand, trade policies, or shifts in consumer preferences can influence farming decisions. Pesitides that might contain toxic chemicals that reduce yields.
